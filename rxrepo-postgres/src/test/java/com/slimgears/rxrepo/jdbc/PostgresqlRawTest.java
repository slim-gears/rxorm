...
//                .blockingAwait();
    ...
    public void testCreateSchema() throws SQLException {
        ...
        execute(statementProvider.forInsertOrUpdate(Inventory.metaClass, Inventory.create(UniqueId.inventoryId(1)), referenceResolver));
        execute(statementProvider.forInsertOrUpdate(Product.metaClass, Product.create(UniqueId.productId(1), 100)
                .toBuilder()
                .type(ProductEntity.Type.ComputeHardware)
                .inventory(Inventory.create(UniqueId.inventoryId(1)))
                .build(),
                referenceResolver));
    }

    private void execute(SqlStatement statement) throws SQLException {
